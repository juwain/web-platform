...
Паттерн Strategy (Стратегия) похож на <a href="/web-platform/trainers/design-patterns-js/10-factory-theory">паттерн Factory</a> подходом. В Factory в зависимости от поданного на вход параметра создаётся и возвращается объект определённого типа. А в Strategy — выполняется определённый для каждого типа параметра произвольный набор действий.
...
- если ветвей становится много, они перемешиваются и «шарят» логику между собой, становится сложно вычленить, что именно относится к отдельной ветке → при внесении правок в ветку растёт вероятность появления ошибки в соседней (цикломатическая сложность)
...
- можно менять стратегии в рантайме
